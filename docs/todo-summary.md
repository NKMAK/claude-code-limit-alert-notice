...
| 状態 | タスク | 概要 | 詳細doc |
|---|---|---|---|
| [ ] | トリガー方式の切替 | launchd(5分) と Stopフック(応答直後) を `.env` フラグ（`TRIGGER_LAUNCHD`/`TRIGGER_HOOK`）でON/OFF。`usage-alert.sh` に `--source` 引数を追加 | [docs/todo-trigger-switch.md](todo-trigger-switch.md) |
