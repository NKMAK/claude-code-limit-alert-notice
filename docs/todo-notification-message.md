...
## 追加TODO: 通知閾値を `.env` で配列設定（整備）

### 現状
**すでに `.env` の `THRESHOLDS` で設定可能**。スペース区切りの「配列」を
`usage-alert.sh` 内で `for th in $THRESHOLDS` で回している。
```sh
THRESHOLDS="50 80"   # 50% と 80% で通知
```
状態ファイルによる重複排除も閾値の値ごとなので、任意の個数・値で動く。
→ 機能は存在するが「明文化・堅牢化」が未了。

### やること（整備）
- [ ] `.env.example` と README に `THRESHOLDS` の書式と例を明記
      （例: `THRESHOLDS="50 80 95"` で3段階通知）
- [ ] 入力の正規化: **カンマ区切りも許容**（`"50,80"` → 空白に変換）。前後空白も除去。
- [ ] バリデーション: 各要素が 1〜99 の整数か検証。昇順ソート＆重複除去。
      不正値は無視 or ログに警告。
- [ ] 値が空/未設定なら既定 `"50 80"` にフォールバック（現状の挙動を維持）。
- [ ] 動作確認: 1個 / 3個以上 / カンマ区切り / 不正値混在 のケース。

### 実装メモ
- 正規化例: `THRESHOLDS=$(printf '%s' "$THRESHOLDS" | tr ',' ' ')`
- ソート/重複除去: `echo $THRESHOLDS | tr ' ' '\n' | grep -E '^[0-9]+$' | sort -n -u`
